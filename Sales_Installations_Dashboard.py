...
def render_dashboard(df, wb, role, region):
# Page Config
# --------------------------------------------------
st.set_page_config(page_title="Sales & Installations Dashboard", layout="wide")
...
st.caption(f"Latest month available in data: {latest_month}")
    ...
    # 👉 Paste your full dashboard charts/tables here
    st.subheader("📈 Sales & Installation Performance")
    st.dataframe(df.head())
    st.subheader("🔄 Winback Performance")
...
if not st.session_state.authenticated:
    ...
    if st.button("Login"):
        ...
        if not user_row.empty:
            ...
            st.session_state.region = user_row["Region/City"].values[0]
        else:
